...
var inventory: Inventory = Inventory.with_capacity(30)
...
func add_item_to_inventory(item_id: ItemID.ID, quantity: int = 1, quantity_from_world: int = 0) -> bool:
	...
	# 查询是否已经有该item，尝试堆叠
	for item: ItemSlot in inventory.filter(ItemSlot.is_occupied):
	...
	if quantity_left > 0:
		for item: ItemSlot in inventory.filter(ItemSlot.is_empty):
			item.reset()	# 可以移除
			item.id = item_id
